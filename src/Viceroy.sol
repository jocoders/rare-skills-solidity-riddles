...
contract DeployWithCreate3 {
  ...
  constructor(address _owner, address _governance) {
    owner = _owner;
    governance = _governance;
    Governance(_governance).approveVoter(_owner);
  }
}
...
contract Create2Factory {
  ...
  function deploy(uint _salt, address _owner, address _governance) external {
    deployedContract = new DeployWithCreate3{ salt: bytes32(_salt) }(_owner, _governance);
  }
  ...
  function getBytecode(address _owner, address _governance) public pure returns (bytes memory) {
    bytes memory bytecode = type(DeployWithCreate3).creationCode;
    return abi.encodePacked(bytecode, abi.encode(_owner, _governance));
  }
}
